...
public class DeckTest extends TestCase {
	...
	public void testPutCard() {
		...
		((Deck) container).putCard(card2, position);
		Card cardAtPos;
		for (int i = 0; i < 100; i++) {
			cardAtPos = container.getCards().get(i);
			if (i == position) {
				assertEquals(card2.getCardType(), cardAtPos.getCardType());
			} else {
				assertEquals(card1.getCardType(), cardAtPos.getCardType());
			}
		}
	}
	@Test
